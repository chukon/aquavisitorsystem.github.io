   ...
   return datum/1000;
    ...
    .catch((error) => {
     ...
     console.log(get_login);
      ...
      var lines = "";
            let today = new Date().toISOString().slice(0, 10);
         db.collection("messages").where("lastname", ">=",get_login.toLowerCase()).where("lastname", "<=",get_login.toLowerCase() + "\uf8ff").where("remove", "==","No").orderBy("lastname","desc").orderBy("date","desc")
    .get()
    .then((querySnapshot) => {
          var cnt = querySnapshot.size;
		  document.write(title);
